...
[[noreturn]] void error(const char *msg)
    ...
    try
    {
        ...
        int yes = 1;
        int bindResult = setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int));
        bindResult = bind(sockfd, (struct sockaddr *) &server_addr, sizeof(server_addr));
        if ( bindResult < 0)
        {
            throw SocketError();
        }
    ...
    ServerSocket(const int &port, const string ip = "127.0.0.1" ): PORT{port}, IP{ip} { initSocket(port); }
